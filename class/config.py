...
class config:
    ...
    def user_mail_send(self, get):
        ...
        # 测试发送
        if self.mail.qq_smtp_send(get.email.strip(), '告警测试邮件', '告警测试邮件'):
    ...
    def set_dingding(self, get):
        ...
        if self.mail.dingding_send('告警测试'):
            return public.returnMsg(True, '添加成功')
        else:
            ret = []
            public.writeFile(self.__dingding_config, json.dumps(ret))
 ...
 # 取Session缓存方式
    ...
    def set_local(self,get):
        ...
        tmp_path = '/bin/bash /www/server/panel/'
        if os.path.exists(d_path):
            t_str = '关闭'
            os.remove(d_path)
            public.ExecShell('chmod -R 777 ' + tmp_path + "notLine.sh")
            public.ExecShell(tmp_path + "notLine.sh")
        else:
            t_str = '开启'
            public.writeFile(d_path,'True')
            public.ExecShell('chmod -R 777 ' + tmp_path + "line.sh")
            public.ExecShell(tmp_path + "line.sh")
