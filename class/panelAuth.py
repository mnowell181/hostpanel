...
class panelAuth:
    ...
    def create_serverid(self,get):
        try:
            ...
            if not os.path.exists(userPath): return public.returnMsg(False,'请先登陆官网用户')
            tmp = public.readFile(userPath)
            if len(tmp) < 2: tmp = '{}'
            data = json.loads(tmp)
            if not data: return public.returnMsg(False,'请先登陆官网用户')
            if not 'serverid' in data:
                data['serverid'] = self.get_serverid()
                public.writeFile(userPath,json.dumps(data))
            return data
        except: return public.returnMsg(False,'请先登陆官网用户')
    ...
    def get_plugin_price(self,get):
        try:
            ...
            if not os.path.exists(userPath): return public.returnMsg(False,'请先登陆官网帐号!')
            params = {}
