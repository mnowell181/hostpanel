		...
		$.ajaxSetup({ 
			...
			error: function(jqXHR, textStatus, errorThrown) {
				...
				if(jqXHR.status == 500 && (jqXHR.responseText.indexOf('运行时发生错误') != -1 || error_find != -1)){
					if(jqXHR.responseText.indexOf('请先绑定帐号!') != -1){
					...
					if(error_find != -1){
						...
						<ul class="help-info-text">\
							...
							<li style="list-style: none;">2、如上述尝试未能解除此错误，请截图此窗口到论坛发贴寻求帮助, 论坛地址：<a class="btlink" href="#" target="_blank">#</a></li>\
						</ul>\
					</div>'
...
var Term = {
    ...
    run: function (ssh_info) {
		...
		// 	layer.msg('当前为开发者模式，不支持终端，请在【面板设置】页面关闭开发者模式!',{icon:2,time:5000});
		// 	return;
		// }
	...
	dialog_chat_show:function(_message){
		...
		$('.chat-content').append('<div class="chat-dialog-box">\
			...
			<div class="chat-dialog-border">\
				<label style="'+(_message.type==1?"margin-left:16px":"")+'">客服 '+_message.date+'</label>\
