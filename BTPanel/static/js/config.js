	...
	layer.open({
		...
		content: '<div class="bt-form pd20 pd70" style="padding:20px 35px;">\
                ...
                <div class="details" style="margin-top:5px;padding-left: 3px;">\
					...
					<label style="font-weight: 400;margin: 3px 5px 0px;" for="check_port">我已了解</label>,<a target="_blank" class="btlink" href="#">如何放行端口？</a>\
				</div>\
			</div>',
...
$('#show_recommend').click(function(){
	...
	layer.confirm(status?'关闭活动推荐，将无法接受到官方推荐的活动内容？':'开启活动推荐，定期获取官方推荐的的活动内容！',{title:status?'关闭活动推荐':'开启活动推荐',closeBtn:2,icon:13,cancel:function(){
...
$('#show_workorder').click(function(){
	var status = !$(this).prop("checked"),that = $(this);
	layer.confirm(status?'关闭在线客服，将无法实时向技术人员反馈问题？':'开启BUG反馈，实时向技术人员反馈问题？',{title:status?'关闭在线客服':'开启在线客服',closeBtn:2,icon:13,cancel:function(){
		that.prop("checked",status);
	}}, function() {
			...
			content: '<div class="bt-form pd20 pd70 ssl_cert_from" style="padding:20px 35px;">\
				<div class="">\
					...
					<ul style="width:91%;margin-bottom:10px;margin-top:10px;">\
						...
						<li>请先下载APP或(谷歌认证器)，并完成安装和初始化</li>\
						<li>基于google Authenticator 开发,如遇到问题请点击<a target="_blank" class="btlink" href="#">了解详情</a></li>\
					</ul>\
				</div>\
	...
	layer.open({
        ...
        content: '<div class="bt-form" style="padding:40px 25px; 20px 25px">\
					<div class="verify_item">\
						<div class="verify_vice_title" style="font-weight: 500;font-size:16px;">扫码绑定（使用面板APP或Google身份验证器APP扫码）</div>\
						<div class="verify_conter" style="text-align:center;padding-top:10px;">\
							<div id="verify_qrcode"></div>\
						</div>\
					</div>\
					<div class="verify_tips">\
						<p>提示：请使用“ 面板APP或Google身份验证器APP ”绑定,各大软件商店均可下载该APP，支持安卓、IOS系统。<a href="#" class="btlink" target="_blank">使用教程</a></p>\
						<p style="color:red;">开启服务后，请立即使用“面板APP或Google身份验证器APP”绑定，以免出现无法登录的情况。</p>\
					</div>\
				</div>',
		success:function(e){
        ...
        bt.send('get_cert_source', 'config/get_cert_source', {}, function (rdata) {
            ...
            var _data = {
                ...
                list: [
                  ...
                  {
                  	html:'<div class="details"><input type="checkbox" id="checkSSL" /><label style="font-weight: 400;margin: 3px 5px 0px;" for="checkSSL">'+lan.config.ssl_open_ps_4+'</label><a target="_blank" class="btlink" href="#">'+lan.config.ssl_open_ps_5+'</a></p></div>'
                  }
	...
	$.post('/config?action=GetPanelSSL',{},function(cert){
		...
		var certBody = '<div class="tab-con">\
			...
			<ul class="help-info-text c7 pull-left">\
				<li>粘贴您的*.key以及*.pem内容，然后保存即可<a href="#" class="btlink" target="_blank">[帮助]</a>。</li>\
	...
	var rhtml = '<div class="boxConter" style="display: none">\
					<div class="iconCode" >\
						<div class="box-conter">\
							...
							<div class="codeTip">\
								<ul>\
									<li>1、打开面板小程序<span class="btlink weChat">小程序二维码<div class="weChatSamll"><img src="#"></div></span></li>\
									<li>2、使用小程序扫描当前二维码，绑定该面板</li>\
								</ul>\
        ...
        layer.open({
			...
			content: ' <div class="bt-form bt-form" style="padding:15px 25px">\
                        ...
                        <ul class="help-info-text c7">\
                            ...
                            <li>API接口文档在这里：<a class="btlink" href="#" target="_blank">#</a></li>\
                        </ul>\
					</div>'
		})
    });
    ...
    $.post('/config?action=get_basic_auth_stat', {}, function (rdata) {
        ...
        if (rdata.open) {
            ...
            m_html = '<div><i class="layui-layer-ico layui-layer-ico3"></i>'
                ...
                + '<a target="_blank" class="btlink" href="#">什么是BasicAuth认证？</a><p></p></div>'
