    ...
    data_reconstruction: function(data, type, callback) {
        var that = this,
            ...
            info_ps = [
                ...
                ['/www/server', 'PS: 软件安装目录'],
                ['/www/wwwlogs', 'PS: 网站日志目录'],
                ['/www/Recycle_bin', 'PS: 回收站目录,勿动'],
                ['/www/server/panel', 'PS: 主程序目录，勿动'],
                ['/www/server/panel/plugin', 'PS: 插件安装目录'],
                ['/www/server/panel/BTPanel', 'PS: 面板前端文件'],
                ['/www/server/panel/BTPanel/static', 'PS: 面板前端静态文件'],
                ['/www/server/panel/BTPanel/templates', 'PS: 面板前端模板文件'],
        ...
        $.each(data, function(index, item) {
            ...
            if (itemD[0].indexOf('.upload.tmp') != -1) fileMsg = 'PS: 文件上传临时文件,重新上传从断点续传,可删除';
