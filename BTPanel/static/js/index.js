      ...
      init: function () {
          ...
          obj.formatter = function(config){
            ...
            $.each(list,function(index,item){
              _tips += '<span style="display: inline-block;width: 10px;height: 10px;"></span>&nbsp;&nbsp;<span style="'+ (item.indexOf('time') > -1?('color:'+ ((data[item] > 100 && data[item] < 1000)?'#ff9900':(data[item] >= 1000?'red':'#0052d9'))):'') +'">'+  options[item] +'：' +   data[item]  + (item.indexOf('time') > -1 ?' ms':' 次/秒')  +  '</span><br />'
            })
            return _tips;
          }
          obj.list = [];
        ...
        setTimeout(function () {
            bt.system.check_update(function (rdata) {
                ...
                if (rdata.msg.is_beta === 1) {
                    ...
                    $('#btversion').append('<a class="btlink" href="#" target="_blank">  [找Bug奖币]</a>');
                }
    ...
    */
    reander_system_info:function(callback){
        ...
        bt.system.get_net(function (res){
            ...
            $('#diskTime').html(ioTime +' ms').css({'color':ioTime > 100 && ioTime < 1000?'#ff9900':ioTime >= 1000?'red':'#0052d9'})
        ...
        bt.soft.get_index_list(function (rdata) {
            ...
            for (var i = 0; i < rlen; i++) {
                if (rdata[i].setup) {
                    ...
                    var status = '';
                    if (rdata[i].status) {
                        status = '<span style="color:#0052d9" class="glyphicon glyphicon-play"></span>';
                    } else {
            ...
            if (rdata.status === false) {
                ...
                var loading = bt.open({
                    ...
                    content: '<div class="setchmod bt-form">\
                                ...
                                <div class="update_version">当前版本：<a href="#" target="_blank" class="btlink" title="查看当前版本日志">Linux'+ (rdata.msg.is_beta == 1 ? '测试版 ' + rdata.msg.beta.version : '正式版 ' + rdata.msg.version) + '</a>&nbsp;&nbsp;发布时间：' + (rdata.msg.is_beta == 1 ? rdata.msg.beta.uptime : rdata.msg.uptime) + '</div>\
            ...
            if (rdata.status === true) {
                ...
                var loading = bt.open({
                    ...
                    content: '<div class="setchmod bt-form" style="padding-bottom:50px;">\
                                    ...
                                    <div class="update_conter">\
                                        <div class="update_version">最新版本：<a href="#" target="_blank" class="btlink" title="查看版本更新日志">Linux'+ (is_beta === 1 ? '测试版' : '正式版') + rdata.version + '</a>&nbsp;&nbsp;更新日期：' + (result.msg.is_beta == 1 ? result.msg.beta.uptime : result.msg.uptime) + '</div>\
            ...
            layer.open({
                ...
                content: '<div class="bt-form pd20" style="padding-bottom:50px;padding-top:0">\
                            ...
                            <style>\
                                ...
                                .item_list{margin-left:95px;border-left:5px solid #e1e1e1;position:relative;padding:5px 0 0 2px}.index_title{border-bottom:1px solid #ececec;margin-bottom:5px;font-size:15px;color:#0052d9;padding-left:15px;margin-top:7px;margin-left:5px}.index_conter{line-height:25px;font-size:12px;min-height:40px;padding-left:20px;color:#888}.index_date{position:absolute;left:-90px;top:13px;font-size:13px;color:#333}.index_acive{width:15px;height:15px;background-color:#0052d9;display:block;border-radius:50%;position:absolute;left:-10px;top:21px}.index_acive::after{position:relative;display:block;content:"";height:5px;width:5px;display:block;border-radius:50%;background-color:#fff;top:5px;left:5px}\
                            </style>\
                        </div>'
            });
            var countdown = 5;
    ...
    get_warning_list:function(active,callback){
        ...
        bt.send('get_list','warning/get_list',obj,function(res){
            if(res.status !== false){
                ...
                // $('.warning_num').css('color',(that.warning_num > 0?'red':'black')).html(that.warning_num);
