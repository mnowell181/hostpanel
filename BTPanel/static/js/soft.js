...
/**
  ...
  set_soft_tips: function (rdata, type) {
    ...
    if (type === 10) {
      explain.text('安全提醒：第三方插件上架前，官方进行了安全审计，但可能还存在安全风险，在生产环境使用前请自行甄别');
      ...
      explain.text('一键部署已上线，诚邀全球优秀项目入驻(限项目官方) ');
      btn_ground = soft.render_tips_btn(btn_ground, [
        {
          title: '导入项目',
          href: 'javascript:;',
        ...
        $.post('/deployment?action=GetList', pdata, function (rdata) {
            ...
            for (var i = 0; i < rdata.list.length; i++) {
                ...
                zbody += '<tr>'
                    ...
                    + '<td><a class="btlink" target="_blank" rel="noreferrer noopener" href="' + rdata.list[i].official + '">' + (rdata.list[i].author == '' ? rdata.list[i].title : rdata.list[i].author) + '</a></td>'
