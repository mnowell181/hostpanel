		...
		if(rdata == []){
			...
			$.post('/crontab?action=GetDataList',{type:'sites'},function(res){
                ...
                for (var i = 0; i < rdata.length; i++){                  
					var s_status = '<span class="btOpen" onclick="set_task_status('+rdata[i].id+',0)" style="color: #2ba471; background-color: #e3f9e9; display: flex; justify-content: center; align-items: center; height: 25px; border-radius: 5px;cursor:pointer" title="停用该计划任务">正常<span  class="glyphicon glyphicon-play"></span></span> ';
					var optName = '';
					if(rdata[i].status!=1) s_status = '<span onclick="set_task_status('+rdata[i].id+',1)"  class="btClose" style="color: #d54941; background-color: #fff0ed; display: flex; justify-content: center; align-items: center; height: 25px; border-radius: 5px;cursor:pointer" title="启用该计划任务">停用<span style="color:rgb(255, 0, 0);" class="glyphicon glyphicon-pause"></span></span> ';
...
$(".dropdown ul li a").click(function(){
	...
	$('#logs_tips').remove();
	$(".plan-submit").css({"pointer-events":"auto","background-color":"#0052d9","color":"#fff"});
	switch(type){
		case 'day':
			closeOpt();
			toHour();
...
function toShell(){
	...
	if($("b[val='toShell']").text() === '同步时间'){
		...
		shell_body = 'echo "|-正在尝试从ntp.aliyun.com同步时间..";\n\
ntpdate -u ntp.aliyun.com\n\
if [ $? = 1 ];then\n\
	echo "|-正在尝试从time1.cloud.tencent.com同步时间..";\n\
...
function message_channel_start(){
	$.post('/config?action=get_settings',function(res){
		...
		if(!s_mail && !s_ding){
			...
			return $(".message_start").html('<span style="color:red;">未设置消息通道，请前往面板设置添加消息通道配置<a href="#" target="_blank" class="bt-ico-ask" style="cursor: pointer;">?</a></span>');
		}
		if(s_mail){
