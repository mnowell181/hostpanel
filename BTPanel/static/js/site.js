...
var site = {
        ...
        var add_web = bt_tools.form({
            ...
            form: [{
                ...
                group: [{
                    ...
                    change: function(value, form, that, config, ev) {
                        console.log(form['codeing'])
                        if (value['sql'] === 'MySQL') {
                            form['datauser'].parents('.line').removeClass('hide');
                            form['codeing'].parent().parent().removeClass('hide');
                        } else {
                            form['datauser'].parents('.line').addClass('hide');
                            form['codeing'].parent().parent().addClass('hide');
                        }
                    }
                }, (function() {
            ...
            form: [{
                ...
                group: {
                    ...
                    list: [
                        ...
                        '案例：test.cn:8081|/www/wwwroot/test.cn|1|1|56'
                    ]
                }
            }]
        });
    ...
    del_site: function(wid, wname, callback) {
        var num1 = bt.get_random_num(10,20),num2 = bt.get_random_num(10,20),title = '';
                ...
                if (auth_type == 'dns' && res.status === 'pending') {
                    ...
                    setTimeout(function() {
                        ...
                        $('.div_txt_jx').append(bt.render_help([
                            ...
                            '若您使用的是云解析插件,阿里云DNS,DnsPod作为DNS,可使用DNS接口自动解析'
                        ]));
                    });
                    return;
                }
                                        ...
                                        var load_form = bt.open({
                                            ...
                                            success: function() {
                                                ...
                                                for (var i = 0; i < datas.length; i++) {
                                                    ...
                                                    var _other = (bt.os == 'Linux' && i == 0) ? '<span>规则转换工具：<a href="#" target="_blank" style="color:#20a53a">Apache转Nginx</a></span>' : '';
                ...
                var _tabs = [{
                        ...
                        callback: function(robj) {
                            bt.pub.get_user_info(function(udata) {
                                if (udata.status) {
                                    ...
                                    robj.append('<div style="margin-bottom: 10px;" class="alert alert-success">此品牌证书适合生产项目使用，官网也是用这款证书，性价比高，推荐使用</div>\
                                        <div class= "mtb10" >\
                                        ...
                                        <li><a style="color:red;">申请www.baidu.cn这种以www为二级域名的证书，需绑定并解析顶级域名(baidu.com)，否则将验证失败</a></li>\
                                                ...
                                                $.each(list, function(index, item) {
                                                    if (bt.check_domain(item)) {
                                                        ...
                                                        switch (index) {
                                                            ...
                                                            case 1:
                                                                if (item.indexOf('*') != 0) {
                                                                    set_info_tips(el, { msg: '通配符域名格式错误,正确写法‘*.baidu.com’', color: 'red' });
                                                                    status = false;
                                                                }
                                                                break;
                                                            case 2:
                                    ...
                                    function check_url_txt(url, content, _this) {
                                        ...
                                        bt.send('check_url_txt', 'ssl/check_url_txt', { url: url, content: content }, function(res) {
                                            loads.close();
                                            var html = '<span style="color:red">失败[' + res + ']</span><a href="#" target="_blank" class="bt-ico-ask" style="cursor: pointer;">?</a>'
                                            if (res === 1) {
                                    ...
                                    function reader_domains_cname_check(data) {
                                        ...
                                        if (data.type) {
                                            ...
                                            for (var i = 0; i < paths.length; i++) {
                                                check_html += '<tr><td><span title="' + paths[i].url + '" class="lib-ssl-overflow-span-style">' + paths[i].url + '</span></td><td>' + (paths[i].status == 1 ? '<a class="btlink">通过</a>' : '<span style="color:red">失败[' + paths[i].status + ']</span><a href="#" target="_blank" class="bt-ico-ask" style="cursor: pointer;">?</a>') + '</td><td style="text-align:right;"><a href="javascript:bt.pub.copy_pass(\'' + paths[i].url + '\');" class="btlink">复制</a> | <a href="' + paths[i].url + '" target="_blank" class="btlink">打开</a> | <a data-url="' + paths[i].url + '" data-content="' + data.info.fileContent + '" class="btlink check_url_results">重新验证</a></td>'
                                            }
                                            ...
                                            html = '<div class="lib-ssl-parsing">\
                                                ...
                                                check_html +
                                                '<div class="parsing_tips" style="font-size:13px;line-height: 24px;">· 请确保以上列表所有项都验证成功后点击【验证域名】重新提交验证</br>· 如长时间验证不通过，请通过【修改验证方式】更改为【DNS验证】</br>· SSL添加文件验证方式 ->> <a href="#" target="_blank" class="btlink" >查看教程</a> <span style="padding-left:60px">专属客服QQ：' + data.info.kfqq + '</span></div>\
                                            ...
                                            html = '<div class="lib-ssl-parsing">\
                                                ...
                                                <div class="parsing_tips" style="font-size:13px;line-height: 24px;">· 请确保以上列表所有项都验证成功后点击【验证域名】重新提交验证</br>· 如长时间验证不通过，请通过【修改验证方式】更改为【DNS验证】</br>· 如何添加域名解析，《<a href="#" class="btlink" target="__blink">点击查看教程</a>》，和咨询服务器运营商。</div>\
                                                    ...
                                                    <div class="business_line">\
                                                        ...
                                                        <div class="business_info business_advantage">\
                                                            ...
                                                            <div class="business_advantage_item" style="width:75%">\
                                                                <span class="advantage_icon glyphicon glyphicon glyphicon-ok"></span>\
                                                                <span class="advantage_title">官方推荐(官方也是使用的该证书)</span>\
                                                            </div>\
                                                        </div>\
                                                    </div>\
                                                    <div class="business_line">\
                                          ...
                                          userLoad.close();
                                            ...
                                            if (config.code.indexOf('multi') > -1) {
                                                if (config.code.indexOf('wildcard') > -1) {
                                                    placeholder = '多域名通配符证书，每行一个域名，支持' + config.limit + '个域名，必填项,例如：\r*.test.cn';
                                                } else {
                                                    placeholder = '多域名证书，每行一个域名，支持' + config.limit + '个域名，必填项,例如：\rwww.hostpanel.cc';
                                                }
                                                ...
                                                if (config.code.indexOf('wildcard') > -1) {
                                                    placeholder = '请输入需要申请证书的域名（单域名通配符证书），必填项，例如：*.test.cn';
                                                } else {
                                                    placeholder = '请输入需要申请证书的域名（单域名证书），必填项，例如：www.hostpanel.cc';
                                                }
                                    ...
                                    robj.append('<div class="alert alert-warning" style="padding:10px">未绑定账号，请注册绑定，绑定账号(非论坛账号)可实现一键部署SSL</div>');
                                    var datas = [
                                        { title: '账号', name: 'bt_username', value: rdata.email, width: '260px', placeholder: '请输入手机号码' },
                                    ...
                                    robj.append(bt.render_help(['商用证书相对于普通证书，具有更高的安全性、赔付保障和支持通配符和多域名等方式。<a class="btlink" target="_blank" href="https://www.racent.com/sectigo-ssl">点击查看</a>', '已有账号请登录绑定']));
                                }
                            });
                        }
                    },
                                ...
                                if (udata.status) {
                                    bt.site.get_domains(web.id, function(ddata) {
                                        ...
                                        var helps = [
                                            ...
                                            'SSL申请的是免费版TrustAsia DV SSL CA - G5证书，仅支持单个域名申请',
                                            '有效期1年，不支持续签，到期后需要重新申请',
                                            '建议使用二级域名为www的域名申请证书,此时系统会默认赠送顶级域名为可选名称',
                                            '在未指定SSL默认站点时,未开启SSL的站点使用HTTPS会直接访问到已开启SSL的站点',
                                            'SSL申请注意事项及教程 <a href="#" target="_blank" class="btlink"> 使用帮助</a>'
                                        ]
                                    ...
                                    robj.append(bt.render_help(['SSL证书为亚洲诚信证书，需要实名认证才能申请使用', '已有账号请登录绑定', 'SSL申请的是TrustAsia DV SSL CA - G5 原价：1900元/1年，用户免费！', '一年满期后免费颁发']));
                                }
                            });
                        }
                    },
                        ...
                        callback: function(robj) {
                            ...
                            if (rdata.status && rdata.type == 1) {
                                ...
                                var helps = [
                                    ...
                                    '建议使用二级域名为www的域名申请证书,此时系统会默认赠送顶级域名为可选名称',
                                    '在未指定SSL默认站点时,未开启SSL的站点使用HTTPS会直接访问到已开启SSL的站点',
                                    '如果重新申请证书时提示【订单已存在】请登录官网删除对应SSL订单',
                                ]
                    ...
                    },
                    {
                        ...
                        callback: function(robj) {
                            ...
                            var helps = [
                                '粘贴您的*.key以及*.pem内容，然后保存即可<a href="#" class="btlink" target="_blank">[帮助]</a>。',
        ...
        set_php_version: function(web) {
            bt.site.get_site_phpversion(web.name, function(sdata) {
                ...
                bt.site.get_all_phpversion(function(vdata) {
                    ...
                    var _form_data = bt.render_form_line(data);
                    var _html = $(_form_data.html);
                    _html.append(bt.render_help(['请根据您的程序需求选择版本', '若非必要,请尽量不要使用PHP5.2,这会降低您的服务器安全性；', 'PHP7不支持mysql扩展，默认安装mysqli以及mysql-pdo。',"【自定义】可自定义PHP连接信息，选择此项需填写可用的PHP连接配置","【自定义】当前仅支持nginx，可配合[负载均衡 - 重构版]插件的TCP负载功能实现PHP负载集群","【PHP连接配置】支持TCP或Unix配置，示例：192.168.1.25:9001 或 unix:/tmp/php8.sock"]));
                    $('#webedit-con').append(_html);
                    bt.render_clicks(_form_data.clicks);
        ...
        set_security: function(web) {
            bt.site.get_site_security(web.id, web.name, function(rdata) {
                ...
                var helps = [
                    ...
                    '【许可域名】允许作为来路的域名，每行一个域名,如: www.hostpanel.cc',
    ...
    select_site_txt: function(box) {
        layer.open({
            ...
            area: '600px',
            btn: ['确认', '取消'],
            content: '<div class="pd20"><div class="line "><span class="tname">自定义域名</span><div class="info-r "><input  name="site_name" placeholder="请输入需要申请证书的域名（单域名证书），必填项，例如：www.hostpanel.cc" class="bt-input-text mr5 ssl_site_name_rc" type="text" style="width:400px" value=""></div></div>\
            <ul class="help-info-text c7">\
                    <li> 申请之前，请确保域名已解析，如未解析会导致审核失败(包括根域名)</li>\
                    <li>申请www.hostpanel.cc这种以www为二级域名的证书，需绑定并解析顶级域名(test.cn)，否则将验证失败</li>\
                    <li>SSL证书可选名称赠送规则：</li>\
                    <li>    1、申请根域名(如：test.cn),赠送下一级为www的域名(如：www.hostpanel.cc)</li>\
                    <li>    2、申请当前host为www的域名（如：www.hostpanel.cc）,赠送上一级域名，(如: test.cn)</li>\
                    <li>    3、申请其它二级域名，(如：app.test.cn)，赠送下一级为www的域名 (如：www.app.test.cn)</li>\
                </ul >\
            </div>',
            success: function() {
